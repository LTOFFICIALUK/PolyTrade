/**
 * Key Vault - Secure encryption/decryption for private keys
 * 
 * Uses AES-256-GCM with per-user derived keys for maximum security.
 * Even if the database is compromised, keys cannot be decrypted without
 * the TRADING_KEY_SECRET environment variable.
 */
...
const getMasterSecret = (): string => {
  const secret = process.env.TRADING_KEY_SECRET
  
  if (!secret) {
    throw new Error('[KeyVault] TRADING_KEY_SECRET environment variable is not set')
  }
  
  if (secret.length < 32) {
    throw new Error('[KeyVault] TRADING_KEY_SECRET must be at least 32 characters')
  }
 ...
 * @param userAddress - The user's wallet address (used for key derivation)
 * @returns Encrypted data object safe for database storage
 */
export const encryptPrivateKey = (privateKey: string, userAddress: string): EncryptedData => {
  ...
  const derivedKey = deriveUserKey(masterSecret, userAddress, salt)
 ...
 * @param userAddress - The user's wallet address (must match encryption)
 * @returns The decrypted private key
 */
export const decryptPrivateKey = (encryptedData: EncryptedData, userAddress: string): string => {
  const masterSecret = getMasterSecret()
...
export const secureWipe = (str: string): void => {
  ...
  // This is more of a best-effort than a guarantee
 ...
 * Run: node -e "require('./dist/security/keyVault').generateSecret()"
 */
export const generateSecret = (): string => {
  const secret = crypto.randomBytes(32).toString('hex')
  console.log('\n🔐 Generated TRADING_KEY_SECRET:')
  console.log(`\n   ${secret}\n`)
