...
// Test address for development (can be overridden by client)
const testAddress = process.env.TEST_ADDRESS || '0x1234567890123456789012345678901234567890'
...
async function fetchOrderbook(tokenId) {
  try {
    const response = await fetch(`${POLYMARKET_API}/orderbook/order-book-summary?token_id=${tokenId}`, {
      headers: {
        'Accept': 'application/json',
      },
    })
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'subscribe') {
        const { topic, address, tokenId } = data
        ws._subscribedTopics.add(topic)
        
        // Orderbook doesn't need user address, only tokenId
        if (topic === 'orderbook') {
          if (tokenId) {
            fetchOrderbook(tokenId).then(orderbook => {
              if (orderbook) {
            ...
            ws.send(JSON.stringify({
              topic: 'orderbook',
              error: 'tokenId required for orderbook subscription',
            }))
          }
        } else {
          // Other topics need user address
