...
const TradingPanel = () => {
  ...
  const [buyType, setBuyType] = useState<'yes' | 'down'>('yes')
  const [sellType, setSellType] = useState<'no' | 'up'>('no')
  ...
  const handleBuy = () => {
    if (isBuy) {
      // Toggle between yes and down when Buy is already active
      setBuyType(buyType === 'yes' ? 'down' : 'yes')
    } else {
      // Switch to Buy mode
      setIsBuy(true)
    }
  ...
  const handleSell = () => {
    if (!isBuy) {
      // Toggle between no and up when Sell is already active
      setSellType(sellType === 'no' ? 'up' : 'no')
    } else {
      // Switch to Sell mode
      setIsBuy(false)
    }
  ...
  const isTradingUp = isBuy ? (buyType === 'yes') : (sellType === 'up')
  ...
  return (
    <div className="h-full flex flex-col bg-black max-h-[50vh] lg:max-h-none overflow-y-auto">
      ...
      {executionType === 'limit' && (
        <div className="border-b border-gray-800 p-4 flex-shrink-0 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <span>Buy {buyType === 'yes' ? 'Yes' : 'Down'}</span>
            ...
            >
              <span>Sell {sellType === 'no' ? 'No' : 'Up'}</span>
              <span className={`text-xs font-semibold ${!isBuy ? (isTradingUp ? 'text-green-400' : 'text-red-400') : (isTradingUp ? 'text-green-400' : 'text-red-400')}`}>
                {noPriceFormatted}¢
              </span>
            </button>
      ...
      <div className="p-4 flex-shrink-0">
        ...
        >
          {executionType === 'limit'
            ? `${isBuy ? 'BUY' : 'SELL'} ${isBuy ? (buyType === 'yes' ? 'YES' : 'DOWN') : (sellType === 'no' ? 'NO' : 'UP')} @ LIMIT`
            : `${isBuy ? 'BUY' : 'SELL'} ${isBuy ? (buyType === 'yes' ? 'YES' : 'DOWN') : (sellType === 'no' ? 'NO' : 'UP')}`}
        </button>
      </div>
              ...
              {isEditingQuickTrade ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between pt-1.5 border-t border-gray-800">
                    ...
                    >
                      {isBuy 
                        ? `Buy ${buyType === 'yes' ? 'Yes' : 'Down'}`
                        : `Sell ${sellType === 'no' ? 'No' : 'Up'}`
                      }
                    </button>
                  </div>
                </div>
